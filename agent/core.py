...
def get_page_summary(page, max_elements=25):
    ...
    for el in tagged:
        ...
        scored_elements.append(
            (score, agent_id, tag, placeholder, current_value, text)
        )
    ...
    elements = [
        ...
        for score, agent_id, tag, placeholder, current_value, text
        in scored_elements[:max_elements]
    ]
...
def run_agent(task, start_url, max_steps=10, headless=False, task_id=None):
    ...
    with sync_playwright() as p:
        ...
        for step in range(max_steps):
            ...
            print("Publishing update to:", channel)
            ...
            try:
                if action == "navigate":
                    ...
                    history.append(
                        f'filled element {decision["id"]} with "{decision["value"]}"'
                    )
                    ...
                    history.append(
                        f'pressed {decision["value"]} on element {decision["id"]}'
                    )
                    ...
                    history.append(
                        f'clicked element containing text "{decision["value"]}"'
                    )
    ...
    with open(f"{run_folder}/report.md", "w", encoding="utf-8") as f:
        ...
        f.write(
            f"**Final Answer:** {report['final_answer']}\n\n"
            f"## Steps\n\n"
        )
